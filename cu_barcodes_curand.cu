 ...
 * utilizing cuRAND device API to generate random sequences
 * compile:
 *  nvcc -DTIMEIT -DCUDA_ERROR_CHECK -std=c++11 -maxrregcount=32 -lcuda -lcurand cu_barcodes_curand.cu -o cu_barcodes_curand
...
/* CUDA kernel for setting up the curand random number generator
 * Arguments:
 *  curandState* to an array of curand states
 *  uint32_t numbver of states to generate
 */
__global__ void setup_curand(curandState *d_states, uint32_t CHUNK) {
        
        int ix = threadIdx.x + blockIdx.x*blockDim.x;
        if(ix < CHUNK)
            curand_init(clock64(), ix, 0, &d_states[ix]);
}

/* geneate a random sequence is a CUDA kernel for generating random sequences
 * Arguments:
 *  curandState* to an array of curand states
 *  device char* to hold the sequences
 *  uint32_t number of sequences
 *  uint16_t sequence length
 */
__global__ void generate_random_seqs(curandState *d_states, char* d_seq, uint32_t CHUNK, uint16_t LEN) {
    ...
    if(ix < CHUNK) {
        ...
        for (uint16_t i = 0; i < LEN; i++) {
            local_state = d_states[ix];
            key = (uint8_t)(curand_uniform(&local_state)*(sizeof(charset) - 1));
            d_states[ix] = local_state;
            d_seq[ix*LEN+i] = charset[key];
        }
    }
}
...
/* main expects 3 command line arguments
...
int main( int argc, char** argv ) {
    ...
    // setup curand
    curandState *d_states;  // free later
    CudaSafeCall( cudaMalloc((void **)&d_states, CHUNK*sizeof(curandState)) );
    setup_curand<<<1024, 1024>>>(d_states, CHUNK);
    CudaCheckError();
    ...
    while (!done) {    
        ...
        generate_random_seqs<<<1024, 1024>>>(d_states, d_seq, CHUNK, LEN);
        CudaCheckError();
        
        // copy data to h_seq
        CudaSafeCall( cudaMemcpy(h_seq, d_seq, CHUNK*LEN*sizeof(char), cudaMemcpyDeviceToHost) );

        #ifdef TIMEIT
        t2 = clock();
        fprintf(stderr,"generate_random_seqs\t%f\n", (float)(t2-t1)/CLOCKS_PER_SEC);
        #endif
